...
void main() {
  ...
  group('liveRideProvider', () {
    test(
      'reads the running ride from the store, with nothing else asked',
      () async {
    ...
    test(
      'a running service with no ids in the store reports no ride',
      () async {
    ...
    test(
      'arriving flips the projection mid-ride, with no store re-read',
      () async {
        final service = FakeRideServiceClient(
          running: true,
          originId: 'shahad',
        ...
        expect(
          c.read(liveRideProvider).valueOrNull!.destinationReached,
          isTrue,
        );
        expect(
          c.read(isRideRunningProvider),
          isTrue,
    ...
    test(
      'the service ending the ride on its own clears the projection',
      () async {
        final service = FakeRideServiceClient(
          running: true,
          originId: 'kalyan',
  ...
  group('rideAlertsProvider', () {
    test(
      'reduces a scripted event sequence, and each flag moves alone',
      () async {
        final service = FakeRideServiceClient();
        final c = makeContainer(service);
        ...
        await Future<void>.delayed(Duration.zero);
        expect(
          c.read(rideAlertsProvider).wakeLadderLive,
          isTrue,
    ...
    test('an Extend moves the deadline while liveness never changes', () async {
      ...
      expect(
        c.read(rideAlertsProvider).windDownWindow,
    ...
    test(
      'a live ladder claims the media session, and releases it after',
      () async {
        final service = FakeRideServiceClient();
        final c = makeContainer(service);
        c.read(rideAlertsProvider);
    ...
    test(
      'standing down clears both flags and drops the media session',
      () async {
        final service = FakeRideServiceClient();
        final c = makeContainer(service);
    ...
    test(
      'standing down with no ladder live does not touch the media session',
      () async {
        final service = FakeRideServiceClient();
        final c = makeContainer(service);
        c.read(rideAlertsProvider);
