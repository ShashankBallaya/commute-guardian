...
void main() {
  ...
  // THE PICK CROSSES THE ISOLATE BOUNDARY, or a kill loses her corridor. HIS
  // QUESTION, 10 SEP 2026: "I am on a train from Ghansoli to CSMT and I
  ...
  //  2. draft -> service store `THE CORRIDOR SHE PICKS IS THE ONE THE SERVICE
  //                            IS HANDED` in prepare_and_start_test.dart,
  //                            from a tap on Screen 1, asserted on
  //                            `routeChainPassed`.
  //  3. store -> resumed ride  `A RESUMED RIDE GOES BACK DOWN THE CORRIDOR
  //                            SHE CHOSE` in widget_test.dart, asserted on
  //                            `routeChainPassed`.
  //
  // Link 2 was a SOURCE guard here until 11 Sep 2026, because nothing could
  // drive `prepareAndStart` in a widget test. The harness now does, and
  // removing the corridor from `startRide` fails it on the value, which the
  // source guard could only approximate by reading a line.
